...
//|				  initialize  					  |
//|===============================================|
sampler2D _LukaBackPass, _LukaFrontPass, _LukaFrameData;
uniform float4 _LukaBackPass_ST, _LukaFrontPass_ST, _LukaFrameData_ST;
...
float _ToggleBlockyRGB, _BlockyRGBPush, _ToggleBlockyGlitch, _AllowBGColors, _BGOverlayIntensity, _BGBrokenColorIntensity, _BGOverlayToggle,
	_BlockyGlitchStrength, _BlockyGlitchSpeed, _BlockyRGBSpeed, _BGBrokenRandom, _BDepthX, _BDepthY, _BGMethod;
...
//overlay
float _ToggleOverlay, _UseSepOverlay, _OverlayTransparency, _OverlayXAdjust, _OverlayYAdjust, _OverlayTiling, _OverlayXShift, _OverlayYShift, _OvScOne, _OvScOneY, _OvScOneT, _OvClipOne;
sampler2D _OverlayTexture, _VROverlayTexture;
float4 _OverlayTexture_ST, _VROverlayTexture_ST;
float4 _OvColOne, _OvColTwo, _OvColThree;

//overlay two
float _ToggleOverlayTwo, _OverlayTransparencyTwo,  _OverlayXAdjustTwo, _OverlayYAdjustTwo, _OverlayTilingTwo, _OverlayXShiftTwo, _OverlayYShiftTwo, _OvScTwo, _OvScTwoY, _OvScTwoT, _OvClipTwo;
sampler2D _OverlayTextureTwo;
float4 _OverlayTextureTwo_ST;

//overlay three
float _ToggleOverlayThree, _OverlayTransparencyThree, _OverlayXAdjustThree, _OverlayYAdjustThree, _OverlayTilingThree, _OverlayXShiftThree, _OverlayYShiftThree, _OvScThree, _OvScThreeY, _OvScThreeT, _OvClipThree;
...
//gif overlay
float _ToggleGifOverlay, _OSSRows, _OSSColumns, _OSSSpeed, _OSTotalTiles, _GifTransparency, _GifXAdjust, _GifYAdjust, _GifTiling, _GifXShift, _GifYShift, _GifScrub, _GifClip, _GifMethod;
sampler2D _OverlaySpritesheet;
float4 _OverlaySpritesheet_ST;
float4 _GifColor;
...
float _ToggleFlash, _FlashLeft, _FlashRight, _FlashTop, _FlashBot, _FlashRainbow, _FlashRainbowX, _FlashRainbowY, _FlashRainbowHue, _FlashRainbowSat, _FlashRainbowLight, _FlashRainbowTime;
float4 _FlashColor;
float4 _BlinkColor;
sampler2D _BlinkImage;
float4 _BlinkImage_ST, _BlinkBorder;
float _BlinkMethod, _AnimeSpeed, _AnimeSeed, _BorderShape, _BorderStrength, _BorderEffect, _BorderMethod, _AnimeRadius, _BorderImageX, _BorderImageY, _BorderImageXShift, _BorderImageYShift;
sampler2D _AnimeMap, _BorderMap;
float4 _AnimeMap_ST, _BorderMap_ST;
float4 _AnimeColor, _BorderColor;

//transistion
float _TransX, _TransY, _ToggleTransistion, _ToggleDiagTrans, _TransDL, _TransDR;

//rendering options
float _MirrorRendering, _PlatformRendering;
float _VRAdjust, _PlatformAdjust;

//vr
float _VRLeft, _VRRight;
float4 _VRLeftColor, _VRRightColor;

//screenfreeze
float _ToggleFreeze, _ToggleRFreeze, _ToggleGFreeze, _ToggleBFreeze;
float _FreezeThreshX, _FreezeThreshY;

//motion glitch
float _MotGlitch, _MotGlitchMode, _MotBlockyPower, _MotArtPower, _MotArtSize, _MBForce;
...
float overlayScreens(float currentScreenCol, float addonScreenCol)
{
	return (currentScreenCol < 0.5) ? 2.0*addonScreenCol*currentScreenCol : 1.0 - 2.0*(1.0 - addonScreenCol)*(1.0 - currentScreenCol);
}
...
half4 frag(v2f i, VertexOutput vo, float facing : VFACE) : SV_Target: COLOR
{
	...
	if (getDistanceAway >= _FalloffRange) {
		...
		if (_ToggleRGB) {
			...
			//sampled rgb split
     		...
     		//rgb cells
				...
				int redrows = -1;
				int greenrows = -1;
				float2 rgbgridUVs = float2(0, 0);
				float CELL_SIZE_FLOAT = float(_RGBCells);
				redrows = int(CELL_SIZE_FLOAT / 3.);
				greenrows = _RGBCells - redrows;
				float2 fragCoord = sceneUVs.xy * _ScreenParams.xy;
				float2 p = floor(fragCoord.xy / CELL_SIZE_FLOAT)*CELL_SIZE_FLOAT;
				offsetx = int(fmod(fragCoord.x, CELL_SIZE_FLOAT));
				offsety = int(fmod(fragCoord.y, CELL_SIZE_FLOAT));
				rgbgridUVs = p / _ScreenParams.xy;
				float4 pixelatedUVs = tex2D(_LukaBackPass, rgbgridUVs);
				float4 rgbGridGrab = float4(0, 0, 0, 0);
				if (offsety < _RGBCells - 1) {
					if (offsetx < redrows) rgbGridGrab.r = pixelatedUVs.r;
					else if (offsetx < greenrows) rgbGridGrab.g = pixelatedUVs.g;
					else rgbGridGrab.b = pixelatedUVs.b;
				} 
				grabToChange.r = overlayScreens(rgbGridGrab.r, grabToChange.r);
				grabToChange.g = overlayScreens(rgbGridGrab.g, grabToChange.g);
				grabToChange.b = overlayScreens(rgbGridGrab.b, grabToChange.b);
			}
		...
		UNITY_BRANCH
		if (_ToggleBlockyGlitch && _ToggleBlockyRGB && _BGMethod == 0) {
			//Props to the original blocky glitch ShaderToy author that I based a lot of my noise math on/rewrote their effect! <3
			...
			float3 randomOverlayColor = (1, 1, 1);
		...
		//Effect: Border (Blink, Border, Anime)
		float animeStore = 1;
		UNITY_BRANCH
		if (_BlinkMethod == 0) {
			...
			if (_ToggleFlash > 0) {
				//Effect: Flash
				//to-do: optimization (check notepad for draft of code)
				UNITY_BRANCH
					if (_FlashRainbow > 0) {
						float hueMod = _Time.y*_FlashRainbowTime; //set up time
						hueMod += (sceneUVs.y*_FlashRainbowX); //vertical multiplier
						hueMod += (sceneUVs.x*_FlashRainbowY); //horizontal multiplier
						hueMod *= _FlashRainbowHue;
						float3 hsvColor = smoothHSV2RGB(float3(hueMod, _FlashRainbowSat, _FlashRainbowLight));
						_FlashColor.rgb = lerp(_FlashColor.rgb, hsvColor, _FlashRainbow);
					}
				float newdistX, newdistY;
				float dirX = 0.5 - sceneUVs.x;
				half distX = sqrt(dirX * dirX); 
				float dirY = 0.5 - sceneUVs.y;
				half distY = sqrt(dirY * dirY);
				UNITY_BRANCH
					if (_FlashLeft > 0) {
						if (sceneUVs.x > 0.5) {
							newdistX = smoothstep(.5, 1., _FlashLeft - distX);
							grabToChange = lerp(grabToChange, lerp(grabToChange, _FlashColor, _ToggleFlash), pow(1 - newdistX, 1));
						}
					}
				UNITY_BRANCH
					if (_FlashRight > 0) {
						if (sceneUVs.x < 0.5) {
							newdistX = smoothstep(.5, 1., _FlashRight - distX);
							grabToChange = lerp(grabToChange, lerp(grabToChange, _FlashColor, _ToggleFlash), pow(1 - newdistX, 1));
						}
					}
				UNITY_BRANCH
					if (_FlashTop > 0) {
						if (sceneUVs.y > 0.5) {
							newdistY = smoothstep(.5, 1., _FlashTop - distY);
							grabToChange = lerp(grabToChange, lerp(grabToChange, _FlashColor, _ToggleFlash), pow(1 - newdistY, 1));
						}
					}
				UNITY_BRANCH
					if (_FlashBot > 0) {
						if (sceneUVs.y < 0.5) {
							newdistY = smoothstep(.5, 1., _FlashBot - distY);
							grabToChange = lerp(grabToChange, lerp(grabToChange, _FlashColor, _ToggleFlash), pow(1 - newdistY, 1));
						}
					}
			}
		...
		else if (_BlinkMethod == 1) {
			//rainbow?
			_BorderStrength *= returnOpacity;
			float hueMod = _Time.y*_BlinkRainbowTime;; //set up time
			hueMod += (sceneUVs.y*_BlinkRainbowY); //vertical multiplier
			hueMod += (sceneUVs.x*_BlinkRainbowX); //horizontal multiplier
			hueMod *= _BlinkRainbowHue;
			float3 hsvColor = smoothHSV2RGB(float3(hueMod, _BlinkRainbowSat, _BlinkRainbowLight));
			_BorderColor.rgb = lerp(_BorderColor, hsvColor.rgb, _BlinkRainbow);
			float3 blinkOverlayUVOne = mul(UNITY_MATRIX_V, float4((i.overlayCoordinates.rgb - _WorldSpaceCameraPos), 0)).xyz;
			float2 blinkOverlayUVTwo = (blinkOverlayUVOne.rgb.rg / blinkOverlayUVOne.rgb.b).rg;
			float2 blinkVRuvs = float2(
				((_ScreenParams.b / _ScreenParams.a) //params 
			...
			_BorderColor = lerp(_BorderColor, blinkOverlayVR, _BlinkImagePower);
			float4 frameMaterial = _BorderColor;
			UNITY_BRANCH
			if (_BorderMethod == 1) {
				frameMaterial = grabToChange;
			}
			UNITY_BRANCH
				if (_BorderShape == 0) {
					float testY = abs(sceneUVs.y * 2 - 1);
					float testX = abs(sceneUVs.x * 2 - 1);
					_BorderStrength = 1.0 - _BorderStrength;
					UNITY_BRANCH
						if (_BorderEffect == 0) {
							if (testY >= _BorderStrength > 0 || testX >= _BorderStrength > 0) {
								UNITY_BRANCH
									if (_ForceBlink) {
										return frameMaterial;
									}
									else {
										grabToChange = frameMaterial;
									}
							}
							else {
								UNITY_BRANCH
						...
						else if (_BorderEffect == 1) {
							if (testY < _BorderStrength && testX < _BorderStrength) {
								UNITY_BRANCH
									if (_ForceBlink) {
										return frameMaterial;
									}
				...
				else if (_BorderShape == 1) {
					float2 vigUVs = sceneUVs * (1. - sceneUVs);
					float vignette = (pow(vigUVs.x*vigUVs.y*15., 1));
					UNITY_BRANCH
					if (_BorderEffect == 0) {
						if (vignette < _BorderStrength) {
							UNITY_BRANCH
								if (_ForceBlink) {
									return frameMaterial; //color
								}
								else {
									grabToChange = frameMaterial;
								}
				...
				else if (_BorderShape == 2) {
					float3 blinkOverlayUVOne = mul(UNITY_MATRIX_V, float4((i.overlayCoordinates.rgb - _WorldSpaceCameraPos), 0)).xyz;
					float2 blinkOverlayUVTwo = (blinkOverlayUVOne.rgb.rg / blinkOverlayUVOne.rgb.b).rg;
					float2 blinkVRuvs = float2(
						((_ScreenParams.b / _ScreenParams.a) //params 
							* blinkOverlayUVTwo.r * _BorderImageX) + _BorderImageXShift //x axis 
						, (blinkOverlayUVTwo.g * _BorderImageY)) + _BorderImageYShift	 //y axis 
						+ 0.5;
					float testColor;
					testColor = tex2D(_BorderMap, blinkVRuvs).r;
					if (blinkVRuvs.x > 1 || blinkVRuvs.x < 0 || blinkVRuvs.y > 1 || blinkVRuvs.y < 0) {
						testColor = 0;
					}
					UNITY_BRANCH
						if (_BorderEffect == 0) testColor = 1 - testColor;
					UNITY_BRANCH
						if (_BorderMethod == 0) {
							grabToChange = lerp(grabToChange, frameMaterial, testColor);
						}
						else {
							grabToChange = lerp(tex2D(_LukaFrontPass, sceneUVs), frameMaterial, testColor);
						}
				}
		}		
		else if (_BlinkMethod == 2) {
			float3 blinkOverlayUVOne = mul(UNITY_MATRIX_V, float4((i.overlayCoordinates.rgb - _WorldSpaceCameraPos), 0)).xyz;
			float2 blinkOverlayUVTwo = (blinkOverlayUVOne.rgb.rg / blinkOverlayUVOne.rgb.b).rg;
			float2 blinkVRuvs = float2(
				((_ScreenParams.b / _ScreenParams.a) //params 
			...
			_AnimeColor = lerp(_AnimeColor, blinkOverlayVR, _BlinkImagePower);
			...
			hueMod *= _BlinkRainbowHue;
			float3 hsvColor = smoothHSV2RGB(float3(hueMod, _BlinkRainbowSat, _BlinkRainbowLight));
			_AnimeColor.rgb = lerp(_AnimeColor, hsvColor.rgb, _BlinkRainbow);
					...
					half2 screenCenter;
					if (sceneUVs.x < 0.5) {
						screenCenter = half2(0.25, 0.5);
					}
					else {
						screenCenter = half2(0.75, 0.5);
					}
			#else
					half2 screenCenter = half2(0.5, 0.5);
			#endif

			//no radius = 1.5
			//normal anime radius = _AnimeRadius
			_AnimeRadius = 1.5 + (returnOpacity - 0) * (_AnimeRadius - 1.5) / (1 - 0.5);
			half dist = sqrt(dir.x * dir.x + dir.y * dir.y);
			float newDistance = smoothstep(.5, 1., _AnimeRadius - dist);
			//get center of screen then normalize
			float2 normalizedCenter = (normalize(sceneUVs - screenCenter) + (_AnimeSeed + (_Time.y*_AnimeSpeed)));
			//applying to a noise map, like reverse screen distortion?
			float4 appliedNoise = tex2D(_AnimeMap, TRANSFORM_TEX(normalizedCenter, _AnimeMap));
			//lerping between the noise map, screen, and distance from the center
			float test = (appliedNoise.rgb*(1 - newDistance));
			grabToChange = float4(lerp(grabToChange.rgb, _AnimeColor.rgb, (appliedNoise.rgb*(1 - newDistance))), grabToChange.a);
		}
		...
		if (_ToggleOverlay) {
			...
			* _OverlayTiling + 0.5;
			...
			overlayFixedUV *= _OvColOne;

			//clipping off alpha
			clip(overlayFixedUV.a - _OvClipOne);
			...
			if (_ToggleOverlayTwo) {
				...
				* _OverlayTilingTwo + 0.5;
				...
				//clipping off alpha
				clip(overlayFixedUVTwo.a - _OvClipTwo);

				overlayFixedUVTwo *= _OvColTwo;
			...
			if (_ToggleOverlayThree) {			
				...
				* _OverlayTilingThree + 0.5;
				...
				//clipping off alpha
				clip(overlayFixedUVThree.a - _OvClipThree);

				overlayFixedUVThree *= _OvColThree;
		...
		UNITY_BRANCH
		if (_ToggleGifOverlay && _GifMethod == 0) {
			...
			* _GifTiling + 0.5;
			...
			float _oneUVMath = round(((_OSSRows*_OSSColumns)*frac((_OSSSpeed*_storeTime.g)+_GifScrub)));
			...
			//clipping off alpha
			clip(_gifScreenUV.a - _GifClip);

			//applying color
			_gifScreenUV *= _GifColor;
		...
		//Effect: Motion Glitch
		UNITY_BRANCH
			if (_MotGlitch) {
				UNITY_BRANCH
					if (_MotGlitchMode == 0) {
						float1 randOverlayColor;
						randOverlayColor = smoothstep((glitchNoiseGenerator(sceneUVs, 1, 40.0, _Time.y * 3, _BlockGlitchMap)), 0, 1);
						grabToChange = lerp(grabToChange, float4(tex2D(_LukaFrameData, sceneUVs)), clamp((randOverlayColor*2)*_MotBlockyPower, 0, 1));
					}
					else if (_MotGlitchMode == 1) {
						float3 randomOverlayColor = (0, 0, 0);
						randomOverlayColor.r = smoothstep((glitchNoiseGenerator(sceneUVs, 1, 40.0*_MotBlockyPower, _Time.y * 3, _BlockGlitchMap)), 0, 1);
						randomOverlayColor.g = smoothstep((glitchNoiseGenerator(sceneUVs, 2, 30.0 / _MotBlockyPower, _Time.y * 2, _BlockGlitchMap)), 0, 1);
						randomOverlayColor.b = smoothstep((glitchNoiseGenerator(sceneUVs, 3, 20.0*_MotBlockyPower, _Time.y * 1, _BlockGlitchMap)), 0, 1);
						grabToChange.r = lerp(grabToChange.r, float4(tex2D(_LukaFrameData, sceneUVs)).r, randomOverlayColor.r);
						grabToChange.g = lerp(grabToChange.g, float4(tex2D(_LukaFrameData, sceneUVs)).g, randomOverlayColor.g);
						grabToChange.b = lerp(grabToChange.b, float4(tex2D(_LukaFrameData, sceneUVs)).b, randomOverlayColor.b);
					}
					else if (_MotGlitchMode == 2) {
						//inspired by shadertoy shader "mpeg artifacts"
						float2 blockSize = float2(_MotArtSize, _MotArtSize);
						float2 adjustedUVs = sceneUVs.xy * _ScreenParams.xy;
						float2 block = floor(adjustedUVs.xy / blockSize);
						float2 uv_noise = (block / 64) * floor(_Time.y * 2000 / blockSize);
						float block_thresh = pow(frac(_Time.y), 2.0) / 1000;
						if (tex2D(_BlockGlitchMap, uv_noise).r * _MotArtPower < block_thresh) {
							grabToChange = float4(tex2D(_LukaFrameData, sceneUVs));
						}
					}
					else if (_MotGlitchMode == 3) {
						//inspired by shadertoy shader "mpeg artifacts"
						float2 blockSize = float2(_MotArtSize, _MotArtSize);
						float2 adjustedUVs = sceneUVs.xy * _ScreenParams.xy;
						float2 block = floor(adjustedUVs.xy / blockSize);
						...
						if (tex2D(_BlockGlitchMap, uv_noise).r * _MotArtPower < block_thresh) {
							float3 randomOverlayColor = (0, 0, 0);
							randomOverlayColor.r = smoothstep((glitchNoiseGenerator(sceneUVs, 1, 40.0*uv_noise.x, _Time.y * 3, _BlockGlitchMap)), 0, 1);
							randomOverlayColor.g = smoothstep((glitchNoiseGenerator(sceneUVs, 2, 30.0*uv_noise.y, _Time.y * 2, _BlockGlitchMap)), 0, 1);
							randomOverlayColor.b = smoothstep((glitchNoiseGenerator(sceneUVs, 3, 20.0*uv_noise.x, _Time.y * 1, _BlockGlitchMap)), 0, 1);
							grabToChange.r = lerp(grabToChange.r, float4(tex2D(_LukaFrameData, sceneUVs)).r, randomOverlayColor.r);
							grabToChange.g = lerp(grabToChange.g, float4(tex2D(_LukaFrameData, sceneUVs)).g, randomOverlayColor.g);
							grabToChange.b = lerp(grabToChange.b, float4(tex2D(_LukaFrameData, sceneUVs)).b, randomOverlayColor.b);
						}
					}
					else if (_MotGlitchMode == 4) {
						//overlay color
						float3 randomOverlayColor = (0, 0, 0);
						randomOverlayColor.r = smoothstep((glitchNoiseGenerator(sceneUVs, 1, 40.0, _Time.y * 3, _BlockGlitchMap)), 0, 1);
						randomOverlayColor.g = smoothstep((glitchNoiseGenerator(sceneUVs, 2, 30.0, _Time.y * 2, _BlockGlitchMap)), 0, 1);
						randomOverlayColor.b = smoothstep((glitchNoiseGenerator(sceneUVs, 3, 20.0, _Time.y * 1, _BlockGlitchMap)), 0, 1);
						float testCol = ((randomOverlayColor.r + randomOverlayColor.g) - (randomOverlayColor.b*(1 + _MotBlockyPower)));
						grabToChange = lerp(grabToChange, float4(tex2D(_LukaFrameData, sceneUVs)), testCol);
					}
			}

		//Effect: Screenfreesze
		UNITY_BRANCH
			if ((_FreezeThreshX + _FreezeThreshY) > 0) {
				UNITY_BRANCH
					if (_ToggleFreeze) {
						UNITY_BRANCH
							if (_MBForce == 0) {
									if (sceneUVs.x > _FreezeThreshX && sceneUVs.y > _FreezeThreshY) {
										grabToChange = float4(tex2D(_LukaFrameData, sceneUVs));
									}
								else {
									if (sceneUVs.x > _FreezeThreshX && sceneUVs.y > _FreezeThreshY) {
										return float4(tex2D(_LukaFrameData, sceneUVs));
									}
								}
							}
					}
				UNITY_BRANCH
					if (_ToggleRFreeze) {
						if (sceneUVs.x > _FreezeThreshX && sceneUVs.y > _FreezeThreshY) {
							grabToChange.r = float4(tex2D(_LukaFrameData, sceneUVs)).r;
						}
					}
				UNITY_BRANCH
					if (_ToggleGFreeze) {
						if (sceneUVs.x > _FreezeThreshX && sceneUVs.y > _FreezeThreshY) {
							grabToChange.g = float4(tex2D(_LukaFrameData, sceneUVs)).g;
						}
					}
				UNITY_BRANCH
					if (_ToggleBFreeze) {
						if (sceneUVs.x > _FreezeThreshX && sceneUVs.y > _FreezeThreshY) {
							grabToChange.b = float4(tex2D(_LukaFrameData, sceneUVs)).b;
						}
					}
			}
			else {
